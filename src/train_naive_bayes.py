...
====================
Training model Gaussian Naive Bayes untuk klasifikasi tingkat obesitas.
...
def train_naive_bayes(X_train_sc, y_train, X_test_sc, y_test,
                      var_smoothing: float = 1e-9) -> dict:
    """
    Melatih model Gaussian Naive Bayes dan mengembalikan dict hasil metrik.
    ...
    y_test       : array-like
    var_smoothing: float — smoothing variansi (default scikit-learn: 1e-9)
    ...
    print("  Prior probabilitas kelas (dari data train):")
    for cls, prior in zip(ORDER_TARGET, model.class_prior_):
        print(f"    {cls:30s}: {prior:.4f}")
...
def main():
    ...
    result = train_naive_bayes(X_train_sc, y_train, X_test_sc, y_test)

    # Simpan model
    model_path = os.path.join(MODELS_DIR, "naive_bayes.joblib")
    joblib.dump(result["model"], model_path)
    print(f"\n✓ Model Naive Bayes disimpan ke: {model_path}")
