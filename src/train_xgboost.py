...
train_xgboost.py
================
Training model XGBoost Classifier untuk klasifikasi tingkat obesitas.
...
def train_xgboost(X_train_sc, y_train, X_test_sc, y_test,
                  n_estimators: int = 200,
                  learning_rate: float = 0.05,
                  max_depth: int = 6) -> dict:
    """
    Melatih model XGBoost dan mengembalikan dict hasil metrik.
    ...
    y_test        : array-like
    n_estimators  : int   — jumlah boosting rounds
    learning_rate : float — learning rate (eta)
    max_depth     : int   — kedalaman maksimum setiap pohon
    ...
    print("  Top-10 Feature Importance (F-score):")
...
def main():
    ...
    result = train_xgboost(X_train_sc, y_train, X_test_sc, y_test)

    model_path = os.path.join(MODELS_DIR, "xgboost.joblib")
    joblib.dump(result["model"], model_path)
    print(f"\n✓ Model XGBoost disimpan ke: {model_path}")
